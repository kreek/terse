...
describe('eval score acceptance', () => {
	...
	it('scores only the named deliverable and includes offline grammar findings', () => {
		const evalCase = {
			...
			arms: {
				with: [{ files: {
					'notes.md': 'She goes home.',
					'notes-outline.md': 'We utilize this record.',
				} }],
				without: [{ files: { 'notes.md': 'She go home.' } }],
			},
		};
		const result = score([evalCase]);
		expect(result.status).toBe(0);
