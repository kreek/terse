...
your voice consistent, and fit the writing to its reader. The split
exists because a model cannot reliably judge its own prose. The
checker is the outside standard the skills work against.
...
There are three ways to use Terse. Ask Claude to write something,
such as "draft an issue about the login timeout," and it checks its
own draft. Run `/terse:edit docs/design.md` on a file you already
have. Run `/terse:write a design doc for the new scheduler` to start
something new.
...
the rest. In a scripted or one-shot session nobody can approve a
stop. Say so in the request, as in "treat the outline as approved, do
not pause," and the process runs through to the file.
