...
must leave style, structure, and voice exactly as found.
...
- Preserve the author's spelling convention (US or UK) as found;
  consistency within the document beats either standard.
...
- Report every correction as before -> after with a `file:line`
  reference. If nothing needed fixing, say so plainly.
