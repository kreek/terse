...
2. Measure the mechanical profile with
   ...
   average sentence length and its spread, document grade, and the rates of
   adverbs, passive voice, and qualifiers per thousand words. These are the
...
6. On approval, set `status: approved` in the frontmatter. Tell the user
   the template location and that `/terse:edit` and the `writing` skill
   will now honor it.
